...
9. Requirement Type: Functional
    -  Description: The System uses OpenWeatherMap/Weather.gov API to request wind data local to the user.
    ...
    -  Fit Criterion: The system can successfully request accurate data from the API for a users location. If it cannot it will display a relevant error message.
    -  Priority: Necessary
    -  Dependencies: 

10. Requirement Type: Functional
    -  Description: The system generates a stand grade by comparing a stand's degree with the most recent wind data.
    -  Rationale: A grading system for stands is an intuitive way for the user to know which stands are advantageous.
...
15. Requirement Type: Functional
    -  Description: Within the Calendar Dashboard view, the system will display the Lunar Phase for each date, potentially with a visual icon to represent the current phase.
    -  Rationale: For various reasons, some hunters prefer to hunt during specific phases of the moon whether its for visibility or a perceived effect on animal movement. This feature would let the user plan hunts around the         lunar calendar.
...
16. Requirement Type: Performance
...
17. Requirement Type: Performance
...
18. Requirement Type: Performance
...
19. Requirement Type: Performance
...
20. Requirement Type: Look and Feel
  -  Description: The application will provide a toggle for "Dark Mode". When active the interface will switch from the default background to a background with a dark background and text.
  -  Rationale: This feature allows the user to access the web application in low light conditions such as dawn or dusk without a bright white screen degrading their night vision and causing a glare.
  -  Fit Criterion: (A) Navigate to the settings and toggle the Dark theme. (B) Verify that the main background color and text of the web app changes white still remaining legible. 
  -  Priority: Optional
  -  Dependencies: N/A
...
22. Requirement Type: Usability
    ...
    -  Fit Criterion: (A) Create a stand facing north. (B) Feed in mock wind data to say that wind is coming from the east. (C) Verify on the dashboard that the icon correctly displays the stand and wind direction.
    -  Priority: Optional
    -  Dependencies: 
...
23. Requirement Type: Performance
...
25. Requirement Type: Security
    -  Description: The system will store all user password hashes.
    -  Rationale: To protect user credentials from a data breach or an unauthorized database access
    -  Fit Criterion: Inspecting the database of user information will verify that the password field contains hashed strings rather than the user's actual password.
    -  Priority: Necessary
    -  Dependencies: 
...
26. Requirement Type: Security
    ...
    -  Rationale: To prevent unauthorized users from accessing the application and user data
    -  Fit Criterion: A user entering valid credentials is redirected to their account and dashboard. A user entering invalid credentials receives an error message and remains on the login page.
    -  Priority: Necessary
    -  Dependencies: 
